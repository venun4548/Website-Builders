  ...
  useEffect(() => {
    ...
    if (pusherKey && !pusherKey.includes('mock')) {
      try {
        ...
        console.warn('Pusher ticket error:', err);
      }
    }
  }, [selectedTicket?.id]);
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSelectedTicket(data.ticket);
        setTickets((prev) =>
          prev.map((t) => (t.id === selectedTicket.id ? { ...t, status: data.ticket.status } : t))
        );
      }
    } catch (err) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <div className="hidden lg:flex items-center gap-4 text-xs font-semibold">
              ...
              <a href="/admin/tickets" className="text-blue-400 bg-blue-500/10 px-3 py-1.5 rounded-lg">
                Tickets
              </a>
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col space-y-6">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Client inquiries and support threads synchronized in Google Sheets
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-[550px]">
            ...
            <div className="lg:col-span-8 bg-slate-900/70 border border-slate-800 rounded-3xl flex flex-col overflow-hidden">
              {selectedTicket ? (
                <>
                  <div className="p-4 sm:p-6 border-b border-slate-800 bg-slate-900/90 flex flex-wrap items-center justify-between gap-4">
                    <div>
                        ...
                        >
                          ...
                          >
                            <div className="flex items-center justify-between gap-4 mb-1 text-[10px] opacity-75">
                              <span className="font-semibold">{isClient ? selectedTicket.clientName : 'Staff Reply'}</span>
                  ...
                  <form onSubmit={handleSendReply} className="p-4 border-t border-slate-800 bg-slate-950/60 flex items-center gap-3">
                    <input
                      ...
                      placeholder="Type response to client (will send instant email alert)..."
                ...
                <div className="m-auto text-xs text-slate-500">Select a ticket to view thread</div>
              )}
            </div>
          </div>
        )}
