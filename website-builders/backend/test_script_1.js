    ...
    function viewEnquiryDetails(enquiryId) {
      try {
        if(!rawEnquiryData) { alert('Data not loaded'); return; }
        const enq = rawEnquiryData.find(e => String(e.id) === String(enquiryId));
        if (!enq) {
          alert('Enquiry not found! ID: ' + enquiryId + ' | Data length: ' + rawEnquiryData.length);
          return;
        }
        ...
        document.getElementById('enquiry-detail-status-badge').className = 'badge ' + (
          enq.status === 'NEW' ? 'badge-primary' :
          enq.status === 'IN_PROGRESS' ? 'badge-warning' :
          enq.status === 'CONVERTED' ? 'badge-success' : 'badge-neutral'
        );
        document.getElementById('enquiry-detail-status-badge').innerText = enq.status.replace('_', ' ');

        document.getElementById('enquiry-detail-status-select').value = enq.status;
        
        document.getElementById('enquiry-detail-customer').innerText = `${enq.full_name} (${enq.email})`;
        document.getElementById('enquiry-detail-mobile').innerText = enq.mobile || 'N/A';
        document.getElementById('enquiry-detail-address').innerText = [enq.address, enq.city, enq.state, enq.zip_code].filter(Boolean).join(', ') || 'N/A';
        document.getElementById('enquiry-detail-message').innerText = enq.message || 'N/A';
        ...
        alert("CRASH in viewEnquiryDetails: " + err.message + "\n" + err.stack);
      }
    }
    ...
    function openConvertEnquiryModal(enquiryId) {
      try {
        ...
        alert("CRASH in openConvertEnquiryModal: " + err.message + "\n" + err.stack);
      }
    }
    ...
    function openAssignStaffModal(userId) {
      try {
        const user = rawPeopleData.find(u => String(u.id) === String(userId));
        if (!user) { alert('User not found in rawPeopleData: ' + userId); return; }
      ...
      openModal('modal-assign-staff');
        ...
        alert("CRASH in openAssignStaffModal: " + err.message + "\n" + err.stack);
      }
    }
